...
async function sendTextMessage(recipientId, text) {
  ...
  if (!res.ok) {
    console.error("Erro ao enviar mensagem via Meta API:", data);
 ...
 * options: [{ title: "Sim", payload: "SIM" }, ...] — máximo 13 opções.
 */
async function sendQuickReplies(recipientId, text, options) {
  ...
  const data = await res.json();
  if (!res.ok) {
    console.error("Erro ao enviar quick replies via Meta API:", data);
    throw new Error(data?.error?.message || "Falha ao enviar mensagem");
  }
  return data;
}
